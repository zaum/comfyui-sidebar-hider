...
- Below that, every toolbar icon is listed with a checkbox, so each icon can
  be shown or hidden individually.
- `Reset to defaults` shows every icon again and turns auto-hide off.
...
- `js/sidebarHider.js` - the whole extension: icon detection, CSS hiding,
  context menu, auto-hide, settings persistence.

## Notes

- Hidden icons are stored in the `SidebarHider.hiddenItems` setting, so they
  survive reloads.
